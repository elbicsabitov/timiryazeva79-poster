...
| KTV-008 | Отправка Татьяне: twinr-zip УШЁЛ 14:42 UTC (msg 589650, без подписи) — но ДО переноса Аудио Метрики, т.е. у неё версия без неё; **нужен повторный send twinr-zip** (пересобран `b4adcc6`: +Аудио Метрика, +twinr-icons) + **CRM-zip не отправлен** (Эльбик сказал «стой») | 🔒 Эльбик-gate (re-send twinr + send CRM) |
