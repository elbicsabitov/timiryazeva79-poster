...
| RUTV-123 | Программы: 25.08 линейка заменена на реальную с ru.tv/programms (9 программ, описания с сайта); сетка вещания и финальные тексты — за редакцией (см. RUTV-137) | 🔒 заказчик |
...
| RUTV-127 | Прод-тайминги баллов: в демо ускорено ×5 (60с вместо 300с) — вернуть при интеграции. 25.08: бонус-сундук удалён правкой Марго (#617118 п.3), остался только +10-тик; пилюля баллов в топбаре скрыта Стасом 26.08 (баланс на Витрине, тикер в PiP) | open |
...
### Showcase v2.10–v2.11 — финальные волны 25-26.08 (Марго DFM #617118 + Стас 617668-676; АКТУАЛЬНОЕ в Saved: zip 617679 + HTML 617680)

**v2.10 (25.08, коммит `6ef5653`) — 8 пунктов Марго (#617118, реплай на v2.9):** (1) чарт «Супер 20» нумерацией вертикально — grid-auto-flow column: главная 1-5/6-10, полная двадцатка 1-10/11-20, разделитель над верхом правого столбца погашен nth-child; (2) соцсети +«Like»+«RuTube» (круги 52→46px, гэп 10 — пять в колонку 300px; RUTUBE малым кеглем); (3) кнопки «Смотреть эфир» и бонус-сундук под трансляцией сняты — сундук-механика удалена целиком (CSS+JS+правило «+50» на витрине), +10-тик и Витрина живы, эфир = клик по экрану/PiP; (4) счётчик просмотров с трансляции снят; (5) «Программы» = двухколонник: сетка слева (поднята), Телепрограмма блоком 360px справа, ≤1100px программы первыми; (6) программы заменены на реальную линейку ru.tv/programms — 9 шт (Супер 20 · Тема · Статус: в сети · RUновости · Дискотека Дискотек · Топ-чарт Rocket Records · Сияй по-звёздному · Личный вопрос · Дотянись до звёзд) с краткими описаниями, сжатыми из текстов сайта (вытащены из Nuxt-пейлоада `window.__NUXT__`; ведущие настоящие: Курдюкова/Юрьев/Голден/Мустафаева), карточки 4:3, телепрограмма пересобрана (реальные слоты: «Статус: в сети» будни 13:00-15:00, «Дискотека Дискотек» вс 22:00); (7) «Ностальгия» → «Подборки» (route-id внутренний); (8) стили = Glass + белый: Apple Dark удалён (19 CSS-блоков + кнопки), сохранённый «apple» из localStorage гасится гардом. Следствие-аудит: демо-события/видео со снятыми программами переименованы, креатив со вшитым «Звёзды Хайпа» (Rectangle 4322-2.png) заменён нейтральными 4322-3/-9/-10 (класс ловушки «чужой текст в кадре» — как SHAMAN-инцидент).

**v2.10.1 (26.08, коммит `967f5bc`) — бренд-сверка по вопросу owner'а:** «Like» из правки Марго = **Likee** (RU.TV ведёт там аккаунт — их новость про челлендж в Likee; написание бренда с двумя «e») → глиф и футер Like→Likee; RuTube сверен: официальный канал rutube.ru/u/rutv, вордмарк RUTUBE.

**v2.11 (26.08) — две правки Стаса после форварда (DFM 617668-676):** (а) «задний фон в стеклянной теме на что-то нейтральное… может градиенты» → glass-фон = нейтральный градиент (мягкие брендовые свечения #E40087/#7a2be2/#21B5FF на тёмной базе) вместо кадра артиста; затемняющий ::after и мёртвый dynamic-backdrop удалены; (б) «вот это в правом верхнем углу скроем» (фото 617674 = пилюля баллов ⭐) → пилюля баллов из топбара скрыта; механика жива: баланс на Витрине подарков (пункт меню), тикер в PiP. Верификация обеих волн: DOM-аудит числом, гард-тест apple→glass, 2 темы × 1920 + ~500px, консоль 0 (одна строка = известный артефакт resize из evaluate), standalone 0 asset-refs, node --check 0. **v2.12 (26.08) — «Программы» по схеме Стаса (617724-726, «как на DFM в твоей стилистике», dfm.ru/schedule):** угловой вариант v2.11.1 отклонён (617722 «да нее») → референс декодирован скрином: у DFM расписание = матрица неделей (строки-дни с датами, плитки «название + время + длительность», активный слот залит красным). Перенос в нашу стилистику: телепрограмма-матрица СВЕРХУ во всю ширину (7 строк Пн-Вс текущей недели, glass-плитки, «Сейчас» акцент-заливкой с пульс-точкой, длительность вычисляется до следующего слота, ночной «до утра», строки с overflow-x на узких), сетка программ СНИЗУ; двухколонник v2.10 и corner-хак v2.11.1 убраны (расписание сверху — конфликт PiP исчез, угол снова br). Verified: 7 строк с датами недели числом, «Сейчас»=Хиты Дня 11:00·2ч в Ср, обе темы, мобила без h-скролла (строки скроллятся внутри), консоль 0. **v2.13 (26.08, сразу следом):** Стас 617728 (фото-мокап из трёх копий нашего вертикального списка) — «Горизонтально по дням наверное лучше» → матрица транспонирована: дни = колонки 368px в горизонтальной ленте (scroll-snap, тонкий скроллбар), внутри каждой — прежний вертикальный список «время · название · подпись» (стили v2.10 sched-row возвращены как smx-row), «Сейчас» — розовой строкой с бейджем только в сегодняшней колонке; сегодняшняя колонка — розовое inset-кольцо + авто-scrollLeft в кадр; длительности убраны (в мокапе их нет), меты возвращены. Verified: 7 колонок с датами, автопрокрутка к сегодня, «Сейчас»=Статус: в сети 13:00 в Ср, обе темы, мобила (колонки 316px, лента скроллится, страница нет), консоль 0. **v2.13.1 (owner 26.08):** (а) дисклеймер «Сетка — демо: программу передач пришлёт редакция» из видимой ноты убран («это лишнее») — факт остаётся в RUTV-137, нота = «Новости RU.TV — в начале каждого часа»; (б) вопрос «как поведёт себя блок, если значений больше? скролл?» → закреплено: .smx-list max-height 574px + overflow-y auto (тонкий скроллбар, overscroll-contain), сегодняшний список автоподкручивается к «Сейчас» (rect-based); перегруз-тест 20+ слотов: колонка не растёт, скролл внутри, «Сейчас» в кадре; (в) багфикс: авто-позиционирования считались при init на display:none-странице (нулевая геометрия) — renderSched теперь пересчитывается в setRoute при входе на «Программы» (заодно освежая «Сейчас» по часам).

**v2.11.1 (26.08, после приёмки v2.11 Стасом 617685 «супер»; УСТАРЕЛА — отклонена Стасом 617722, снята в v2.12):** Стас 617711 (фото) — PiP на «Программах» ложился поверх телепрограммы → умный дефолт-угол: на «Программах» окошко само встаёт в левый низ (route-aware в updatePip, `pipUserPlaced` после ручного переноса отключает автологику), плюс `padding-bottom: 240px` странице программ — карточки доскролливаются из-под окошка. Проверено синтетикой: bl на программах/br на прочих, drag-выбор переживает смену маршрутов, перекрытие sched = false, последняя карточка чиста при скролле. ⏭ Дальше: реакция DFM (Марго 617703 «отправлять рутв?» — мяч у Стаса) + **перенос в фигму для Лёши после его просмотра** (Стас ЛС 617114; фигма-файл RMG-web-2.0, node 19-2 — ЛС 615864).

| ID | Задача | Статус |
|----|--------|--------|
| RUTV-141 | Glass-градиент подобран по слову Стаса «может градиенты какие-то» (брендовые розово-фиолет-синие свечения) — настроение/палитру сверить по его реакции на файл | open |
| RUTV-142 | Состав соцсетей = VK/OK/MAX/Likee/RuTube; «Like» Марго прочитан как Likee (у RU.TV там аккаунт). Ссылки на сообщества и финальный состав — от редакции (вместе с RUTV-138) | 🔒 заказчик |
| RUTV-143 | Постеры программ = **квадраты 1000×1000** «как на РР первый блок со станциями» (Стас 617787 + Лёша 617789 «квадраты 1000*1000 ок», Стас 617791 «чем меньше ресайзов по всем проектам, тем проще всем») — карточки программ переведены 4:3 → 1:1 в v2.14 (перекрывает «чуть горизонтальные» Марго 25.08). Сами квадратные постеры перерисует и пришлёт RU.TV (Стас 617788 «я попрошу перерисовать») | 🔒 заказчик |
| RUTV-144 | **Внутрянки от Лёши** (617812): страница программы · «Обратная связь» в Контактах · плеер потока/материала со всеми контролами · внутрянка новости · персоны (артист/ведущий) · крошки + «Назад» | **done 2026-08-26 (v2.15)** — все 7 экранов построены в прототипе и капчерены в фигму ×4 варианта (см. блок v2.15 ниже) |
| RUTV-146 | Плеер: качество-меню (Авто/1080/720/480), громкость, mute, перемотка (VOD), fullscreen/PiP-кнопки = демо-механика прототипа; реальный плеер при имплементации | open |
| RUTV-147 | Выпуски программ, тексты внутрянок новостей, био артистов/ведущих, связки ведущий↔программа — демо; реальный контент пришлёт редакция (вместе с RUTV-122/137) | 🔒 заказчик |
| RUTV-148 | **Внутрянки по образцу сайтов РМГ** (owner 04.09 в DFM: «клод неочень делает, надо корректить по примерам прошлых проектов»): референсы декодированы скринами (rusradio.ru/shows, /persons, /news, /djs; ru.tv/programms, /air/vjs; dfm.ru/news, /mediateka, /podcasts) и перенесены в нашу стилистику | **done 2026-09-08 (v2.16)** — см. блок v2.16 |
| RUTV-149 | Ведущие = реальный состав ru.tv/air/vjs (Иван Чуйков, Макс Кобзов, Мария Валуйская, Ксения Акчурина, Артём Иванов, Катя Голден, Самира Мустафаева) + ведущие программ из описаний ru.tv/programms (Антон Юрьев, Софья Курдюкова). Связки программа→ведущий: 4 сорсированы (Супер 20/Дискотека Дискотек/Сияй/Дотянись), остальные 5 — демо; фото и био пришлёт RU.TV | 🔒 заказчик (закрывает половину RUTV-122) |
| RUTV-150 | Демо-наполнение внутрянок v2.16: гости выпусков (кроме «Дискотеки Дискотек» — с ru.tv), цитаты-врезки новостей, био артистов, «live на Премии РУ.ТВ»/«Личный вопрос» в клипах артиста, «Источник \ Фото» — заменить редакционным | 🔒 заказчик (вместе с RUTV-147) |
| RUTV-151 | Рекламное место 300×600 на внутрянках (правая колонка программы/новости/артиста/ведущего — как на rusradio.ru и ru.tv) — тот же формат, что у эфира; мобильный формат — RUTV-124 | open |
| RUTV-152 | **Формат картинки новости = ОДИН, 16:9 (1920×1080)** для карточки и статьи (Лёша 622390 «квадрат или прямоугольник — внутри кропнется, я бы выбрал одно»; Стас 622401 «как на РР» — обложка статьи rusradio.ru = `aspect-ratio 1920/1080`, замерено живьём). Редакции готовить обложки новостей в 16:9; квадратные/портретные демо-фото кропаются с фокусом `news[].pos` (Дискотека `center 15%`) | done 2026-09-09 (v2.17) — 🔒 заказчик: подтвердить формат редакции |
| RUTV-153 | Тег-чипы над сеткой «Новости» (фильтр по теме, как rusradio.ru/news): темы = теги демо-новостей (Премьера/Концерт/Чарт/Сервис/Спецпроект); реальный рубрикатор — от редакции | open |
| RUTV-154 | Кнопка «Назад» снята (Лёша 622390: крошки ИЛИ назад) — навигация назад = кликабельные крошки; при имплементации браузерная история (pushState) закрывает системную «назад» | open |

**v2.15 (26.08 ночь, «го все» owner'а) — ВНУТРЯНКИ ЛЁШИ ПОСТРОЕНЫ + ФИГМА-СИНК 36 ФРЕЙМОВ:** по 617812 построены: (1) **страница программы** (крошки, квадратный арт, мета+описание, чипы слотов из телепрограммы (scheduleSlotsFor: «Будни 13:00»), кнопки «Смотреть эфир»/«Напомнить», 6 демо-выпусков 16:9); (2) **плеер потока** (route live-view; вход: PiP-expand/клик по окошку + новая кнопка ⤢ на hero главной; контролы: play/pause, громкость+mute, LIVE-пилюля, качество-меню, «Свернуть в окошко», fullscreen; «Далее в эфире» = 4 слота из сетки с «сейчас»; PiP на этой стр. скрыт); (3) **плеер материала** (route watch; theater-ширина по высоте вьюпорта; прогресс с ползунком/буфером и кликом-перемоткой (время пересчитывается), громкость, качество, PiP/fs-кнопки, ♥/Поделиться, «Смотрите также»; PiP скрыт — играет материал; открывается с: video-грида, новинок, клип-каруселей подборок (data-open-clip JSON), строк Супер 20 (лайки не перехватываются), выпусков программ); (4) **внутрянка новости** (чип+заголовок+мета+обложка 16:9+лид жирным+3 абзаца (news[].body добавлены всем 6) + «Читайте также» 3 карточки); (5) **персона-артист** (аватар, чип «№N в Супер 20»→чарт, клипы-фильтр по имени + новости-фильтр; фолбэк «Сейчас в ротации»); (6) **персона-ведущий** (монограмма, «Ведёт программы» — 2 карточки демо-связкой; правка редакции №8 про связь на личной странице закрыта); (7) **Контакты: вкладка «Обратная связь»** (форма имя/почта/тема-селект/сообщение + чекбокс согласия; демо-отправка с валидацией и тостом). **Навигация**: routeStack + «Назад», крошки на всех внутрянках, делегат кликов (динамические data-route/data-open-*, статические помечены data-rb), Enter/Space открывает карточки. Verified: честный флоу-тест кликами (12 проверок: программа→выпуск→watch→назад×2, новость, артист (№1+клипы 1+новости 1), ведущий (2 программы), ⤢→live (LIVE-пилюля, no-seek, PiP скрыт), minify→назад, чарт-строка→watch + лайк НЕ переходит, клип-рубрика→watch, форма (без согласия=тост-отказ, с=успех+reset), крошки-ссылка, Enter), обе темы глазом, мобила 390 (hScroll 0 везде, volbar скрыт, форма 1 колонка), консоль: 0 ошибок (2 issue autocomplete → добавлены атрибуты), node --check 0, standalone 0 asset-refs (7.34MB). **Фигма = 36 капчеров одной сессией** (4 параллельные Chrome-страницы, ID пачками): 28 новых фреймов (11-17 × Glass/Светлая × десктоп/мобила: Программа·внутрянка / Эфир·плеер / Клип·плеер / Новость / Артист / Ведущий / Контакты·ОС) + перекапчерены главная ×4 (добавилась кнопка ⤢) и контакты ×4 (третья вкладка); файл = **69 фреймов**, страница «RU.TV — макеты сайта (v2.15, 26.08)», 36 призрак-сайдбаров удалено, ~2030 нод округлено, топ-дроби 0, финальный скрин всего канваса чист. Капчер-инъект расширен: ?detail= (JSON showcase.detail) и ?ctab= для детальных стейтов. **Поставка: zip → Saved 617832 + голый HTML → Saved 617833** (617821/617822 устарели).

**v2.16 (08.09) — ВНУТРЯНКИ ПО ОБРАЗЦУ САЙТОВ РМГ + ФИГМА-СИНК 28 ФРЕЙМОВ.** Контекст: Стас 03.09 «как успехи с рутв?», 04.09 гиф «жду»; Эльбик 04.09 «в пн-вт отдам внутрянки… надо корректить по примерам прошлых проектов». Референсы (скрины 08.09 в `.scratch/dfm_0908/ref_*.jpeg`): **rusradio.ru/shows** — чипы ведущих под названием, расписание словами, постер справа; **rusradio.ru/persons** — аватар+био, «Новости» рядом из трёх, сетка треков; **rusradio.ru/news** — заголовок + круглая «Поделиться», тег-чипы, чип персоны, обложка 16:9, правая колонка 300×600, «Источник \ Фото» + дата; **ru.tv/programms** — сетка выпусков с гостем, «Больше», список программ справа, «другие программы»; **ru.tv/air/vjs** — портретные карточки ведущих; **dfm.ru/mediateka** — артист чипом/ссылкой. Сделано в `designs/showcase-aggregator.html` (коммит `ab29cee`): (1) **Программа** — двухколонник `.dv-layout` (контент + правая колонка `--promo-w` 300px: «Программы канала» с активным пунктом + рекламное место 300×600 клоном `#promoSlot`), шапка `.pv-hero` (название, чипы ведущих `PROGRAM_HOSTS`, «В эфире по воскресеньям в 22:00» через `humanSlot`, два абзаца `PROGRAM_LONG`, Смотреть эфир/Напомнить/Поделиться, квадратный постер 320px справа — стандарт 1:1 RUTV-143), выпуски = 12 штук с гостем (`PROGRAM_GUESTS` для Дискотеки — реальные с ru.tv; `DEMO_GUESTS` остальным), сетка `.ep-grid-3` auto-fill 290px + play-кружок по hover, показано 8 + «Больше выпусков» (`[data-more-eps]` дорисовывает остаток), «Другие программы» `.sim-row`; (2) **Новость** — `.dv-layout-article` (статья ≤1100px + колонка сразу справа: «Ещё новости» ×4 + реклама; дальний правый край свободен под PiP), `.article-head` с круглой share, `.tag-row` (#тег + #артист + `NEWS_TAGS`), чип артиста → страница артиста, лид + абзац + `<blockquote>` цитата с подписью (`NEWS_QUOTES`) + остаток, `.article-src` «Источник: … \ Фото: … · дата», «Читайте также» ×3; (3) **Артист** — био `ARTIST_BIO` (6 реальных кратких био), чипы №N в Супер 20 (с трендом)/#жанр, «Новости» ×3 (свои + добивка), «Клипы и выступления» (свои клипы + чарт + демо live/интервью, `data-open-clip`), «Сейчас в ротации» ×4, «Похожие артисты» ряд, справа «Супер 20 · сейчас» топ-5 + реклама; (4) **Ведущий** — портрет 3:4 `.host-portrait` (монограмма до фото), роль, чипы программ (`programsOfHost`, фолбэк демо), 2 абзаца `HOST_BIO`, соц-чипы VK/Telegram/MAX (тост «ссылки пришлёт редакция»), «Ведёт программы», «Выпуски с участием» ×4, «Другие ведущие» ряд, справа «Ведущие RU.TV» список + реклама; (5) **Клип** — под заголовком чип артиста + чип «№N в Супер 20 · тренд» + #тег; (6) **Обратная связь** — `.fb-layout`: форма + 3 карточки контактов справа (Редакция/Прямой эфир/Реклама); (7) `hosts` = 9 реальных имён (RUTV-149). Verified: флоу-тест 19 шагов кликами (программа→Больше 8→12→список→ведущий→выпуск→watch→назад→новость (теги 4, персона, цитата, колонка 4)→артист (новости 3, свои 3, ротация 4, чарт 5+1, похожие 7)→чарт→watch→клип→похожий→колонка новостей→тег→тост→ОС 3 карточки→главная, hScroll false), обе темы глазом, мобила 390 (hScroll 0, реклама скрыта ≤1100, колонки в столбик), консоль 0, node --check 0, standalone 7.37MB / 0 asset-refs / 0 не-data фонов. **Поставка: zip → Saved 622250 + голый HTML → Saved 622251** (617832/617833 устарели; Эльбик форвардит сам). **Фигма kn7zZnwAHCdWEzCS1KpIYv: 28 фреймов заменены на месте с прежними именами/координатами** (10/L10 Ведущие, 11/L11 Программа, 13/L13 Клип, 14/L14 Новость, 15/L15 Артист, 16/L16 Ведущий, 17/L17 Контакты·ОС + мобильные M/ML того же набора), 69 фреймов итого, призраки «Sidebar - Меню» удалены, мини-плеер оставлен как в остальных фреймах. ⚠️ Рельсы-ловушки: (а) вкладка из ДРУГОГО окна Chrome даёт капчер 1448px — все капчеры только из окна 1920; (б) фильтр `/sidebar/i` сносит и «Sidebar - Мини-плеер» — удалять ТОЛЬКО `Sidebar - Меню`; (в) per-tab изоляция localStorage для параллельных капчеров = initScript с override `Storage.prototype.getItem/setItem` (query-параметры capture.html пишут в общий localStorage → гонка); (г) get_metadata страницы = 1.6M символов — список фреймов читать `use_figma` (page.children → name/x/y/w/h). **v2.16.1 (08.09, позже, owner: «далее в эфире криво»):** «Далее в эфире» и «Смотрите также» переведены в колонку плеера (`.dv-sec.player-wrap`), слотов 6 в две колонки (`.player-wrap .lv-list` grid, мобила — одна) — раньше список лежал слева на 720px под центрированным плеером. **Поставка: zip → Saved 622253 + HTML → Saved 622254** (622250/622251 устарели). Фигма: 12/L12/13/L13 + M12/ML12/M13/ML13 перекапчерены и заменены (69 фреймов, страница v2.16.1).

**v2.17 (09.09) — ПРАВКИ ЛЁШИ И СТАСА ИЗ DFM 08.09 + ФИГМА-СИНК 32 ФРЕЙМОВ.** Контекст: реплаи на v2.16.1 (622264): Лёша 622390-391 — (1) постер новости «или квадрат, или прямоугольник… внутри кропнется» (скрин: карточка с квадратом + статья 16:9 с обрезанной головой), (2) «крошки ИЛИ кнопка назад — дублирует и путает», (3) «заголовок ряда кликабельный — в Супер 20 теряется переход ко всей двадцатке»; Стас 622401-403 — «давайте как на РР» + скрины rusradio.ru/news (заголовок «Новости ›», тег-чипы, карточки с прямоугольной картинкой сверху) и статьи РР + «только в галерее нижние края скругляем» (на РР у карточки скруглён только верх — проверено зумом); Эльбик 622565 «щас внесу правки и скину». Сделано в `designs/showcase-aggregator.html` (коммит `e6ad605`): (1) **карточка новости «как на РР»** — `.card-news` = обложка 16:9 сверху + подписи снизу, без стеклянного бокса (структура клип-карточек), все 4 угла `--r-lg`, hover scale; главная 4 в ряд (швы с Каналами/Промо), `.news-grid` 4 колонки (≤1100 — 2, ≤900 — 1), «Читайте также»/артист — 3; светлые правила бокса сняты; (2) **один формат картинки 16:9** для карточки и статьи — замер живого rusradio.ru: обложка `aspect-ratio 1920/1080`, `rounded-3xl`, natural 2925×1645 (мастер РР тоже 16:9); `bg()` + `item.pos` (фокус кропа), «Дискотека» `pos: 'center 15%'`; (3) **тег-чипы над сеткой «Новости»** — «Все» + уникальные теги, `newsFilter`/`renderNewsTags()`, делегат `[data-news-tag]`, чип темы в статье ведёт на «Новости» с фильтром; (4) **крошки без «Назад»** — кнопка, `.btn-back`, ветка делегата удалены, `goBack`/`routeStack` только для «Свернуть в окошко»; (5) **заголовки — ссылки с шевроном** — `.row-title a` на главной (Свежие новости/Программы/Супер 20; дубли «Все … →» сняты), `secH3()`/`sideH4()` для внутрянок и боковых карточек (news/video/artists/programs/hosts/chart), «Вся двадцатка →» внизу боковой карточки снят. Verified: флоу-тест 9 шагов кликами (заголовок→раздел, крошка, чип в статье→фильтр 2, «Все»→6, #Чарт→1, «Супер 20»→чарт, боковой h4, карточка сетки→статья, свернуть эфир), обе темы, мобила 390 (hScroll 0), консоль 0, node --check 0, standalone 7.37MB / 0 asset-refs (под file:// 1 сообщение `url(#gp-badge-*)` — как в v2.16.1). **Поставка: zip → Saved 622615 + голый HTML → Saved 622616** (622253/622254 устарели; форвард в DFM + ответ Лёше/Стасу — за owner, драфт в сешн-логе 09.09). **Фигма kn7zZnwAHCdWEzCS1KpIYv: 32 фрейма заменены на месте** (01/05/11/12/13/14/15/16 × Glass/Светлая × десктоп/мобила), 69 итого, призраки 0, топ-дроби 0, страница «RU.TV — макеты сайта (v2.17, 09.09)». Рельса: `http.server 8791` в `designs/` + `capture.html?route=&style=#figmacapture=` в изолированных контекстах chrome-devtools (свой localStorage на вкладку), мобила = emulate до навигации, тема капчера — по яркости первой заливки (0.03/0.96), не по порядку ответов.
| RUTV-145 | Ведущие: на реальном сайте страницы пустые; Лёша заведёт странички, описания и унификацию фото делает RU.TV (617792 + Стас 617793 «да, это тоже они все сделают») — наши карточки ведущих не блокер | 🔒 заказчик |

**v2.14 (26.08 веч., волна DFM после расшаривания фигма-файла) — квадратные постеры программ + синк фигмы:** Лёша 617785-786 (скрин наших карточек + скрин ru.tv: «какой размер? родные постеры широкие») → Стас 617787 фото РР «Как на РР первый блок со станциями» → Лёша 617789 «квадраты 1000*1000 ок» → `.card-program-art` aspect 4:3 → **1:1** (затронуты главная-рейл и сетка «Программы»; структура карточки не менялась — градиент-плейсхолдер с названием внутри, мета+описание снизу). **Фигма-файл kn7zZnwAHCdWEzCS1KpIYv синхронизирован**: 8 фреймов (01/L01/M01/ML01 Главная + 02/L02/M02/ML02 Программы) перекапчерены по той же localhost-рельсе (generate_figma_design с nodeId=0:1 кладёт капчер сразу на страницу; 2 странички Chrome: 1920 окно + эмуляция 390x844x2,mobile,touch; параллельные captureId) и заменены на прежних местах с прежними именами; призрак «Sidebar - Меню» удалён из всех 8; округление прогнано (~1.2к нод, остаток — легитимные FILL-дети и авто-тексты); страница переименована в «RU.TV — макеты сайта (v2.14, 26.08)». Verified: аспект 1.0 числом (рейл 232×232, грид 211×211), обе темы × 1920 глазом, фигма-аудит: 41 фрейм, топ-координаты целые (fracCount 0), скрин-чек фреймов 62:2/64:2 после округления; standalone 0 asset-refs, node --check 0. Поставка: **zip → Saved 617821 + голый HTML → Saved 617822** (617744/617745 устарели). Комменты В САМОМ фигма-файле проверены — их нет (панель пустая); вся волна пришла чатом DFM. ⚠️ В фигме баннер «Your invoice is past due» (план Simbio) — оплату проверить owner'у.
...
| RUTV-138 | Соцсети у эфира: глифы набраны типографикой в кругах — заменить официальными знаками при имплементации (та же рельса, что RUTV-133); ссылки на сообщества пришлёт редакция. 25-26.08: состав расширен до VK/OK/MAX/Likee/RUTUBE (правка Марго #617118 п.2 + бренд-сверка Likee) | open |
| RUTV-139 | «Ностальгия» — рабочее название вкладки подборок по годам | **done 2026-08-25** — Марго (#617118 п.7): «„Ностальгия“ переименовать в „Подборки“» — переименовано (route-id nostalgia остался внутренним) |
