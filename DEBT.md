...
| GOROD-045·047..057 | P1 остаток: Волна 3-зоны (🔒 pixel-perfect gate) · Артист/Трек deep-dive + slop-kill (нужен realign как плеер) · transition-card · edge-glow · P2 loops (recap-карточка, контекст-старты, открытый-профиль, стрики) · P3 backend (reason_tag pipeline, 🔒лицензирование, B2B taste-ads). Детали → AUDIT §8. | pending |
