...
## ⚡ Twinr + CRM Турбо — мобильный адаптив по жалобе Татьяны (2026-07-21)

Бриф = чат Татьяны 21.07 16:03 («по CRM Turbo, и твинеру (ИИ редакция и ТГ аудит) моб адаптив ты делал? …скачала архивы еще раз. но как-то кривовато выглядят»). Диагноз скриншот-проходом на честном 390px (CDP device-emulation — обходит TD-KTV-05): CRM вообще без мобильного слоя (каждый экран 635–667px), twinr — бургер лежал на заголовке всех страниц (регресс от удаления топбара 06-17), таблицы ТГ-аудита с пустыми строками ~250px. Twinr-часть — ветка `feat/twinr-tg-audit` (`8a0d51d`, GATED не merged); CRM — master `30988ac` (PUSHED). Zips в Saved Messages: twinr msg 604576 + CRM msg 604577 (без подписи, форвардит Эльбик).

| ID | Задача | Статус |
|----|--------|--------|
| KTV-028 | Twinr: просвет под плавающий бургер на ВСЕХ страницах (≤920 `.stage` padding-top 68/64 + `--chat-vpad` 96/88; роутер сам делает scrollTo(0)) | done (ветка twinr) |
| KTV-029 | Twinr ТГ-аудит: таблицы без строк-гигантов (описания клампятся 4 строки, `п/н` не ломается, ссылки break-all, отчётные таблицы min-width 480 + перенос) | done (ветка twinr) |
| KTV-030 | Twinr ИИ редакция: фильтры Отбракованных/Опубликованных сеткой (селекты/поиск во всю ширину, даты «с»/«по» парами) вместо «Кан…»/«Жу…» | done (ветка twinr) |
| KTV-031 | Twinr чат на мобиле: `html:has(#page-chat.active)` overflow-lock снят на ≤920 — страница скроллится до композера | done (ветка twinr) |
| KTV-032 | CRM Glass: мобильный слой с нуля — бургер в топбаре + off-canvas сайдбар + scrim (паттерн twinr 06-10), stage без 120px-рельсы, kpi 2/1 кол., таблицы скроллятся в `.t-wrap`, service-cell кламп 3 строки, тач-мишени, модалки full-screen; все 29 экранов = 0 h-overflow на 390 | done (master `30988ac`) |
| KTV-033 | Оба standalone пересобраны + zips в Saved Messages (604576/604577); README в каждом — что смотреть | done |
| TD-KTV-07 | Скриншот-капчер CDP на CRM-страницах таймаутится из-за вечных CSS-анимаций (glass shimmer) — для будущих визуальных прогонов снимать на dpr1 или гасить анимации | note |
...
| TD-KTV-05 | Мобайл-бургер (≤920px плавающая `.nav-burger-float`, тот же #navBurger + drawer-JS) — логика проверена, чистый моб-скрин не снят (тест-окружение DPR 1.65 ломало ресайз); проверить на реальном узком экране | done 07-21 (CDP device-emulation 390×844: дровер открывается/закрывается, скрин снят; заодно найден и закрыт наезд бургера на заголовки — KTV-028) |
