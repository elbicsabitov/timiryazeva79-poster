...
## Update (cont-5) — P2 «Открытый профиль» (052) — pitch-экран
Вход: Эльбик «пушь и гони P2 — открытый профиль 052». Запушил 048/049 (`188336e`+`e8da327` → origin). Затем построил 052.
- **GOROD-052 — `GorodProfile` / роут `#/profile`.** Питч-аргумент против «Яндекс делает то же»: показываем то, что конкуренты прячут умышленно. Вход с `#/taste` («Открытый профиль →», eye-иконка) + прямой URL; НЕ 4-я вкладка (3-tab IA цела). Читает live `gorodfm_taste`+`gorodfm_rejected` (fidelity — показанное = реальный профиль). Контент: (1) **контраст** «чёрный ящик Яндекс/Spotify — grayscale+blur+lock, мёртвый vs открытый профиль — alive+accent»; (2) реальный вектор (5 граней) с **провенансом** по каждой («из онбординга — ты выбрал сам» / behavioral fallback); (3) **1 live-правка** «меньше» → видимая квитанция «волна пересчиталась… профиль слушается тебя» + `TwinrWave.bump`; (4) секция «Что ты оспорил» (rejected chips или empty-state→плеер); (5) «даже реклама — твоя» strip; (6) moat-caption (объяснённые решения нельзя купить ретроспективно).
- **WCAG-фикс (поймал сам до reviewer):** мелкий accent-текст был на бренд-`#5168FC` = 4.25:1 < AA → новый токен `--accent-on-dark #8094ff` (6.7–7.1:1) для kicker/pct/receipt.
- **Verify:** node --check **11/11**; live QA на :8765 (5 граней, правка РОК 84→72%, квитанция, empty-state rejected, 0 console errors); **design-implementation-reviewer ✓ SHIP-READY** (WCAG-AA весь текст: H1 19.7 / facet% 6.7 / kicker·receipt 7.1 / moat 6.3 / closed-box 4.5+; anti-slop clean — closed-box grayscale = намеренная риторика, не slop; mobile 2-кол→1-кол `@media≤720px` без overflow; 0 errors).
- **TD найден (app-wide, backlog):** `TD-GOROD-CTA-AA` — primary-кнопка white-on-#5168FC = 4.43:1 (чуть <AA, 14px/700) — конвенция ВСЕХ кнопок (не вводил в 052), reviewer: accept-not-block; app-wide фикс `#4A5FE8`.
- **Decoupled:** `GorodProfile` = отдельный trailing IIFE; добавлен `#/profile` в VALID_ROUTES + секция в `<main>` + CTA на `#/taste`. Ничего существующего не сломано.
- **Commit + push:** `feat: gorod-fm — «Открытый профиль» pitch screen (GOROD-052)` + standalone rebuild → pushed origin/master.
- **Next:** 045/047 = Эльбик-gate; автономно P2 loops остаток 050/051/053/054.
