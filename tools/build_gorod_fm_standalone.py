    ...
    *skip* lists paths already inlined statically (literal <img>/CSS/JS-string refs).
    ...
    for name in RUNTIME_ASSET_EXTRA:
        ...
        if rel in skip:
            skipped += 1
            continue
        # Only map an extra if the source actually reaches it. dusk-lake.jpg belongs to the
        # chat-layer glass skin; on branches without that skin it would be 327 KB of dead bytes.
        if src_html and name not in src_html:
            unref += 1
            continue
...
def _head_injection(asset_map: dict[str, str]) -> str:
    ...
    script = (
        ...
        "  }\n"
        "  // (0) Expose the resolver. The patches below are REACTIVE: a renderer that builds an\n"
        "  //     innerHTML string holding <img src=\"assets/...\"> loses the race - the HTML\n"
        "  //     parser starts that fetch while the fragment is parsed, before setAttribute/src\n"
        "  //     or any MutationObserver can rewrite it (the image still ends up correct, but a\n"
        "  //     dead file request + console error already happened). Such renderers resolve at\n"
        "  //     the SOURCE via this hook; absent when the dev file is served, so paths stay paths.\n"
        "  try{ window.GOROD_OFFLINE_RESOLVE = rewriteSrc; }catch(e){}\n"
