...
## ⚡ LATEST (2026-07-01) — Город ФМ: HOME STOREFRONT (карусель РМГ + витрина + поиск). 4 модуля СОБРАНЫ+ЗАКОММИЧЕНЫ, передача с owner-фидбеком

**📖 START: `docs/superpowers/HANDOFF-gorod-fm-home-storefront.md`.** Ветка `feat/gorod-home-rmg-storefront` (rebased на чистый `master` 1eab466, БЕЗ chat-layer). HEAD `2aceb42`. Сервер `cd designs && python -m http.server 8791` → `gorod-fm.html#/home`; визуал = chrome-devtools MCP. `check_scripts.cjs` 29 блоков 0 ошибок, 0 console errors.

**Задумка (owner-locked):** главная = НЕ-AI радио-витрина. Карусель=эфир (6 станций РМГ, плоская без наклона, центр=играет), плеер снизу, музыкальная витрина ниже (друзья·категории·подборки·редакция·новинки·чарты·коллекции·исполнители·программы). Весь AI(волна/steer)→ отдельная вкладка «Моя волна» (`#home-radio` закомменчен).

**✅ Собрано:** ресёрч(9/2волны)+спек+план · T1 `GorodStations` · T2 карусель markup+CSS(no-tilt) · T3 рендер+центрирование · T4 живой плеер(LIVE,MediaSession) · T7+8 витрина(9 полок) · T5+6 поиск. T1/T2 ревью ✅; T3–T8 — нужен holistic review.

**🔴 ОСТАТОК (owner-фидбек — детали в HANDOFF):** (1) кнопки play/контролы на ЦЕНТРАЛЬНОЙ играющей карточке «как раньше»; (2) отступы сверху + начальное центрирование + усилить focus карусели; (3) карточки друзей «Слушать то же»→иконки ▶/＋ справа; (4) ФУЛЛ наполнить полки контентом (как онбординг); (5) убрать остатки AI с главной (панель Believer, баннер «почему?», Twinr AI, «Ты дослушал», stale-обложка); (6) поиск визуал+моб-вход; (7) моб-адаптив; (8) holistic ревью; (9) standalone; (10) реал-ассеты (лого/частота/стримы); (11) **чат-рейл «Общий эфир»** — вернуть на новую главную (примирить с full-width витриной) vs оставить на `feat/gorod-chat-layer` (owner флагнул пропажу после rebase на master).

---

## ⚡ PREV (2026-06-17) — Twinr: чат-раздел переделан под fit-in-viewport (правки Татьяны 06-17) — СДЕЛАНО+verified, SYNCED, zip отправлен Татьяне

**Бриф (чат @k_t_v_23 06-17, 3 скрина прода `.scratch/ktv/msg_592182/592193/592194.jpg`):** убрать верхний топбар-стрип ради места (Эльбик «да, думаю норм»); убрать плашку `.section-hero` «Чат» (✕); «Новый чат» под спойлер; чат влезает в экран целиком со всеми кнопками; слово «Чат» снять; чат вынесен в топ-меню («можешь предложить»). Город-ФМ соц-фича из тех же сообщений — НЕ наша (Эльбик: «не в тот чат», параллельная сессия её и делает на feat/gorod-chat-layer).

**Karpathy-ресёрч** (wf 3 агента → синтез → adversarial-критик «ship-with-fixes»): chrome-density / fit-in-viewport AI-chat / progressive-disclosure. Спека+критика → `.scratch/wf_twinr_chat_research.js` (output в tasks/wesfl0mz5).

**Сделано в `designs/twinr-liquid-glass.html` (Chrome-MCP verified, console 0):** (1) топбар удалён глобально → аккаунт+колокольчик в `.sidebar-footer` (collapsed=аватар, hover-reveal; `.nav` scroll-on-hover чтобы подвал не обрезался при низком экране); крошки убраны; бутафорский ⌘K-поиск убран. (2) Чат промоутнут в отдельный nav-item «Чат» (после Статистики, data-route="chat"); убран из AI_TOOLS + routeToNav['page-chat']→'chat' (декапл ДО удаления плашки — иначе injectAiSubnav инжектит ИИ-чипы в чат-сетку). (3) `.section-hero` в чате удалён + `sr-only <h1>` (слово «Чат» снято, routeToTitle сохранён). (4) `#page-chat.active` = вьюпорт-сетка `calc(100dvh - var(--chat-vpad,48px))`, скроллятся только история+поток, композер закреплён снизу; `html:has(#page-chat.active){overflow:hidden}` гасит фантом-скролл смонтированных страниц (LG-008); min-width:0 на pane. (5) Рейл: вкладки Личные/Общие (через `.tabs[data-tabs-group]`/`[data-panel]` handler — zero new JS), «+ Новый чат» = спойлер (button[aria-expanded]+grid-rows анимация), вертикальная история с бейджами модели. (6) Правая панель: тред-бар (имя+чип модели Deepseek) + поток (max-width 820px) + композер (textarea авто-рост + Отправить). Standalone пересобран `tools/build_twinr_crm_standalone.py` (1.70 MB).

**Verified Chrome-MCP:** docScroll 0 · композер всегда виден · текст переносится (без обрезки) · спойлер откр/закр · Личные↔Общие · «Чат» active в меню · аккаунт в подвале (hover) · др. страницы не сломаны (Статистика скроллится, лок только на чате) · 0 console err. **Доставлено:** zip `deliverables/2026-06-17/` отправлен Татьяне (файл без подписи, команда Эльбика). Трекер: DEBT.md «Twinr — чат-редизайн 2026-06-17» (KTV-016..023, TD-KTV-05/06).

**🔴 NEXT:** ждать реакцию Татьяны. ⚠️ Урок: репо ШАРЕД с параллельной gorod-сессией — она переключила ветку на feat/gorod-chat-layer под нами; twinr коммитим на master через stash→checkout master→commit→checkout обратно. Caveat: мобайл-бургер логика-verified (≤920px, тот же drawer-JS), чистый моб-скрин не снят (тест-DPR 1.65 ломал ресайз).

---

## ⚡ PREV (2026-06-11) — Twinr: «Хранилище» = отдельный раздел + MD-конвертер (задача Татьяны) — СДЕЛАНО, доставлено msg 590213
...
**Сделано в `designs/twinr-liquid-glass.html` (Chrome-MCP verified 1440/390/dawn, console 0 err):** (1) `#page-docs` перестроен под прод: upload-карточка (скрепка-пикер с live-именем + «Название (опционально)» + «Загрузить» + лимит 20 МБ) + таблица с её скринов 1:1 (Документ.pdf / 14 АГ Фест mp3 / Картиночка.png + docx-демо), coral-ссылки, сорт-стрелки, per-row «Изменить теги»+«Удалить»; (2) вынесен из ИИ-сабнава в **отдельную вкладку сайдбара** (data-route="storage", archive-иконка; убран из AI_TOOLS); (3) **MD-конвертер**: per-row «В Markdown» (PDF/DOCX) → `modal-md-convert` (PDF→MD, «Преобразовать» со спиннером → «Готово» + выбор «В хранилище»/«В проект» с дропдауном Аудио Метрика/Большой Цифровой/CRM Турбо; обе ветки живые — строка .md первой с флешем / confirm+автозакрытие). Rename Документы→Хранилище везде (routeToTitle/гайд/TOC). Standalone пересобран (1.68 MB).

**✅ ДОСТАВЛЕНО ×2:** v1 msg 590167 (после фикса Эльбика KTV-014: живая загрузка + скачивание .md) → её фидбек 16:20 «надо чтобы БЕЗ загрузки в хранилище, а результат уже в хранилище или в один из проектов» → **KTV-015**: карточка «Преобразовать в Markdown» (файл прямо с компьютера, исходник НЕ сохраняется, автостарт конвертации; per-row вход остался для уже загруженных) → **v2 msg 590213**. Её фидбек по дизайну в целом: «все прям в восторге от дизы )))». **🔴 NEXT:** ждать реакцию Татьяны на v2. Трекер: DEBT.md «Twinr — Хранилище… 2026-06-11» (KTV-010..015, TD-KTV-04).

---

## ⚡ PREV (2026-06-10 вечер) — Monte Carlo (РМГ): промо-блок на главной, 7 концептов в клиентской фигме, ждём выбор клиента
